...
HelloMedia 是独立完整的 **Agent Skill** 包：何时调用写在 `SKILL.md`，实际工作由 `scripts/` 下**纯标准库** CLI 完成。可选安装 [Pillow](https://pypi.org/project/Pillow/)，用于偏清晰度的识图压缩与更稳的剪贴板抓取。

| 模态 | 理解 | 生成 / 编辑 |
|------|:----:|-------------|
| **图像** | `vision.py` / `understand.py`（含 `--from-clipboard`） | `generate.py`（文生图、改图） |
| **视频** | `understand.py --video` | `video.py`（文生 / 图生 / 参考图 / 编辑 / 延长） |
| **音频** | STT（可再经 LLM 摘要） | TTS |

**适合：** 宿主主模型默认不能可靠看图、需要多渠道 API 管线，或必须把生成结果固定落盘时。

**不适合：** 联网搜索（请用搜索类技能）。**默认假设宿主主模型无视觉**，多模态请求应直接走本技能。

### 设计约定

1. **多模态默认走技能** — 识图 / 读视频 / 读音频 / 生图 / 生视频 / TTS / STT 默认直接用 HelloMedia，不要先让主模型试识图。
2. **粘贴需要路径或系统剪贴板** — 宿主 `[Image #N]` 芯片脚本读不到；用 `--image <path>`，或重新复制图像后 `--from-clipboard`。
3. **生成类一律走技能** — 生图 / 生视频 / TTS 使用 `generate.py` / `video.py` / `audio.py`。
4. **自备密钥** — `config.json` 多渠道 + 能力开关；识图/生图不绑定单一厂商。
5. **识图压缩偏清晰度** — 可选 Pillow；默认最长边 2048、JPEG q=90、≤256KB 不压缩。

---

## 功能

- **技能优先路由** — 假设宿主无视觉；理解与生成统一走 HelloMedia
- **剪贴板抓取** — `--from-clipboard`（Pillow / PowerShell / pngpaste / osascript / wl-paste / xclip）；doctor 可探测
...
- **测试** — 离线友好的 `pytest`（能力校验、剪贴板 resolve、压缩、下载、代理、CLI dry-run、路径安全、Sub2API 契约等）
...
- 已开启能力对应的 API Key
- 可选：Pillow（识图压缩 + 更稳的剪贴板）
- 可选剪贴板工具：Windows PowerShell、macOS `pngpaste`、Linux `wl-paste` / `xclip`
...
python scripts/vision.py --from-clipboard --prompt "描述剪贴板截图"
...
**粘贴说明：** 宿主 `[Image #N]` 附件技能脚本读不到。请给磁盘路径，或把图重新复制到 **系统剪贴板** 后执行 `--from-clipboard`。Windows 上贴进 Claude/Grok CLI 常用 **`Alt+V`**。若宿主粘贴已清空剪贴板，需再复制一次图像。
...
| 变量 | 默认 | 含义 |
|------|------|------|
| `HELLOMEDIA_COMPRESS_MIN_BYTES` | 262144 | ≤此大小原样发送（256KB） |
| `HELLOMEDIA_COMPRESS_MAX_SIDE` | 2048 | 仅当最长边超过时才缩边 |
| `HELLOMEDIA_COMPRESS_JPEG_QUALITY` | 90 | 需重编码时的 JPEG 质量（偏清晰） |
| `HELLOMEDIA_COMPRESS_REENCODE_MIN_BYTES` | 2097152 | 未超长边但体积≥2MB 时才允许仅重编码 |
| `HELLOMEDIA_CLIPBOARD_MAX_BYTES` | 41943040 | 剪贴板抓取体积上限 |
...
├── scripts/
│   ├── _common.py           # 共用 HTTP、代理、路径、下载、识图压缩
│   ├── _clipboard.py        # 系统剪贴板取图
...
| 宿主已粘贴但技能“看不到图” | 脚本读不到 `[Image #N]` | 保存路径 → `--image`，或重拷图像 → `--from-clipboard` |
| `clipboard_empty` / 占用 | 剪贴板空、已被消费或锁定 | 重新复制图像；关闭占用程序；或给文件路径 |
| WSL/SSH 剪贴板失败 | 剪贴板命名空间不对 | 存到可访问路径后用 `--image` |
...
- 多模态默认走技能（假设宿主主模型无视觉）
- OS 剪贴板抓取：`--from-clipboard`、`doctor --clipboard`
- 识图压缩偏清晰度：最长边 2048、JPEG q=90、≤256KB 不压缩
- 透明图白底合成；剪贴板占用归为可恢复 empty
